...
def product_search_get_similar_products \
  ...
  index_time = response.responses.first.product_search_results.index_time
  puts "Product set index time:"
  puts "\tseconds: #{index_time.seconds}"
  puts "\tnanos: #{index_time.nanos}\n"

  results = response.responses.first.product_search_results.results
  puts "Search results:"
...
product_search_get_similar_products(*ARGV) if $PROGRAM_NAME == __FILE__
